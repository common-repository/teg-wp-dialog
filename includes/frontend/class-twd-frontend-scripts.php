...
class FWD_Frontend_Scripts {
	...
	public static function get_styles() {
		return apply_filters( 'teg_wp_dialog_enqueue_styles', array(
			...
			'teg-wp-dialog-nifty-modal'    => array(
				'src'     => self::get_asset_url( 'assets/css/nifty-modal.css' ),
				'deps'    => array(),
				'version' => TWD_VERSION,
				'media'   => '',
				'has_rtl' => false,
	...
	private static function register_scripts() {
		...
		$register_scripts =
			array(
				...
				'teg-wp-dialog-nifty-modal-classie'      => array(
					'src'     => self::get_asset_url( 'assets/js/nifty_modal/classie' . $suffix . '.js' ),
					'deps'    => array( 'jquery' ),
					'version' => TWD_VERSION,
				),
				'teg-wp-dialog-nifty-modal-modalEffects' => array(
					'src'     => self::get_asset_url( 'assets/js/nifty_modal/modalEffects' . $suffix . '.js' ),
					'deps'    => array( 'jquery' ),
					'version' => TWD_VERSION,
				),
	...
	public static function load_scripts() {
		...
		if ( $has_shortcode ) {
			...
			self::enqueue_script( 'teg-wp-dialog-nifty-modal-classie' );
			self::enqueue_script( 'teg-wp-dialog-nifty-modal-modalEffects' );
		...
		if ( $enqueue_styles = self::get_styles() ) {
			foreach ( $enqueue_styles as $handle => $args ) {
				...
				if ( $has_shortcode && twd_is_modal_template( $handle ) ) {
