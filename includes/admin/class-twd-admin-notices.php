...
class TWD_Admin_Notices
{
    ...
    {
        self::$notices = get_option('teg_wp_dialog_admin_notices', array());
    ...
    {
        update_option('teg_wp_dialog_admin_notices', self::get_notices());
    }

    /**
     * Get notices.teg_wp_dialog
     * @return array
     */
    ...
    public static function remove_notice($name)
    {
        ...
        delete_option('teg_wp_dialog_admin_notice_' . $name);
    }
    ...
    public static function hide_notices()
    {
        if (isset($_GET['ur-hide-notice']) && isset($_GET['_fd_notice_nonce'])) {
            if (!wp_verify_nonce($_GET['_fd_notice_nonce'], 'teg_wp_dialog_hide_notices_nonce')) {
                wp_die(__('Action failed. Please refresh the page and retry.', 'teg-wp-dialog'));
            }

            if (!current_user_can('manage_options')) {
                wp_die(__('Cheatin&#8217; huh?', 'teg-wp-dialog'));
            }

            $hide_notice = sanitize_text_field($_GET['ur-hide-notice']);
            self::remove_notice($hide_notice);
            do_action('teg_wp_dialog_hide_' . $hide_notice . '_notice');
        }
    }
    ...
    public static function add_custom_notice($name, $notice_html)
    {
        ...
        update_option('teg_wp_dialog_admin_notice_' . $name, wp_kses_post($notice_html));
    }
        ...
        if ($notices) {
            foreach ($notices as $notice) {
                if (empty(self::$core_notices[$notice])) {
                    $notice_html = get_option('teg_wp_dialog_admin_notice_' . $notice);
