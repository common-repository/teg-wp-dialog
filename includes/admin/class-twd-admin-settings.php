<?php
/**
 * TEG WP Dialog Admin Settings Class
 *
 * @author   ThemeEgg
 * @category Admin
...
if ( ! class_exists( 'TWD_Admin_Settings', false ) ) :

/**
 * TWD_Admin_Settings Class.
 */
class TWD_Admin_Settings {
	...
	public static function save() {
		...
		// Trigger actions
		do_action( 'teg_wp_dialog_settings_save_' . $current_tab );
		...
		delete_transient( 'teg_wp_dialog_cache_excluded_uris' );
		TWD()->query->init_query_vars();
		TWD()->query->add_endpoints();
		wp_schedule_single_event( time(), 'teg_wp_dialog_flush_rewrite_rules' );

		do_action( 'teg_wp_dialog_settings_saved' );
	}
...
*@param string $text
	...
	public static function output() {
		...
		do_action( 'teg_wp_dialog_settings_start' );
		...
		$tabs = apply_filters( 'teg_wp_dialog_settings_tabs_array', array() );
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'title':
					...
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_wp_dialog_settings_' . sanitize_title( $value['id'] ) );
					}
					break;
				...
				case 'sectionend':
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_wp_dialog_settings_' . sanitize_title( $value['id'] ) . '_end' );
					}
					echo '</table>';
					if ( ! empty( $value['id'] ) ) {
						do_action( 'teg_wp_dialog_settings_' . sanitize_title( $value['id'] ) . '_after' );
					}
					break;
				...
				case 'multi_select_countries' :
					...
					if ( ! empty( $value['options'] ) ) {
						...
						$countries = TWD()->countries->countries;
					}
				...
				default:
					do_action( 'teg_wp_dialog_admin_field_' . $value['type'], $value );
					break;
			}
		}
	}
...
*@return bool
	...
	public static function save_fields( $options, $data = null ) {
		...
		foreach ( $options as $option ) {
			 ...
			 * @since 1.0.0
			 */
			$value = apply_filters( 'teg_wp_dialog_admin_settings_sanitize_option', $value, $option, $raw_value );

			/**
			 * Sanitize the value of an option by option name.
			 * @since 1.0.0
			 */
			$value = apply_filters( "teg_wp_dialog_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
			 ...
			 * @deprecated 2.4.0 - doesn't allow manipulation of values!
			 */
			do_action( 'teg_wp_dialog_update_option', $option );
		}
	...
	public static function check_download_folder_protection() {
		...
		$downloads_url   = $upload_dir['basedir'] . '/teg_wp_dialog_uploads';
		$download_method = get_option( 'teg_wp_dialog_file_download_method' );
